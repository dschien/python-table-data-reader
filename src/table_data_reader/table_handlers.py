...
class OpenpyxlTableHandler(TableHandler):
    ...
    def check_all_groups_always_present(self, definitions, **kwargs):
        """
        ...
        :param definitions: The definitions dictionary generated by ref_date_handling
        :param groupings: List of groupings from the yaml model
        :return:
        """

        # This is a list of lists. Each list is the countries referenced in a parameter in each scenario? I think??
        cs = []
        ...
        # then, a set is made from the list.
        # Since no duplicate elements in a set, this should reduce to one element if groups are used consistently
        # So, check that this element is the list of groups, and it has a length of 1.
        group_set = set(frozenset(i) for i in cs)
        if len(group_set) != 0 and kwargs.get('groupings'):
            assert set(kwargs.get('groupings')) in group_set
        assert len(group_set) <= 1  # asserts all variables that have group data have the same groupings
    ...
    def load_definitions(self, sheet_name=None, filename: str = None, **kwargs):
        ...
        self.check_all_groups_always_present(definitions, **kwargs)
